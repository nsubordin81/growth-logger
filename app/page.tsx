    ...
    <div style={{ textAlign: 'center', padding: '2rem 1rem' }} className="animate-fade-up">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxWidth: '400px', margin: '0 auto' }}>
          {activeGoals.map(goal => (
            <div key={goal.id} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0.8rem',
              background: goal.status === 'URGENT' ? '#ef444420' : '#f59e0b20',
              borderRadius: '4px', border: goal.status === 'URGENT' ? '1px solid #ef444440' : '1px solid #f59e0b40'
            }}>
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: '0.65rem', fontWeight: 700, color: goal.status === 'URGENT' ? '#ef4444' : 'var(--text)' }}>
                  {goal.priority} {goal.id} • {goal.domain}
                </div>
                <div style={{ fontSize: '0.8rem', fontWeight: 500, color: 'var(--muted)' }}>{goal.title}</div>
              </div>
              {goal.status === 'URGENT' && (
                <span style={{ fontSize: '0.6rem', background: '#ef4444', color: '#fff', padding: '0.15rem 0.4rem', borderRadius: '2px' }}>
                  URGENT
                </span>
              )}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxWidth: '400px', margin: '0 auto' }}>
          {weeklyFocus.map(item => (
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text)' }}>{item.label}</div>
                <div style={{ fontSize: '0.65rem', color: 'var(--dim)' }}>{item.action}</div>
              </div>
              <div style={{ 
                fontSize: '0.65rem', padding: '0.15rem 0.4rem', borderRadius: '2px', fontWeight: 600,
                background: item.status.includes('Not started') ? '#fbbf2420' : '#22c55e20',
                color: item.status.includes('Not started') ? '#fbbf24' : '#22c55e'
              }}>
                {item.status}
              </div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', maxWidth: '400px', margin: '0 auto' }}>
          {domainPerformance.map(d => (
            <div key={d.domain} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text)' }}>{d.domain}</div>
              <div style={{ fontSize: '0.65rem', color: 'var(--green)' }}>{d.quality} quality</div>
              <div style={{ fontSize: '0.6rem', color: 'var(--dim)' }}>{d.time}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: '2rem', padding: '1rem', background: 'var(--surface)', borderRadius: '6px',
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '0.7rem', color: 'var(--dim)' }}>End-of-day Rush</div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--amber)' }}>4/5 ⚠️</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '0.7rem', color: 'var(--dim)' }}>YouTube Distraction</div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--amber)' }}>3/5 ⚠️</div>
          </div>
        </div>
      </div>
...
function SnapshotForm({ onSave, onCancel }: { onSave: (data: any) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ padding: '1rem', maxWidth: '600px', margin: '0 auto' }}>
      ...
      {/* Progress count */}
      <div style={{ marginTop: '2rem', fontSize: '0.6rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
        Clicks so far: <strong>6</strong> (Topic → Domain → Recall → Insight → Challenge)
      </div>
    </div>
  )
}
